...
import { venueRoutes, categoryRoutes, userRoutes } from '@/plugins/routes'
import { Dumbbell, Tag, Users, TrendingUp } from 'lucide-vue-next'

const stats = ref([
  { title: 'Venues', value: '—', icon: Dumbbell, color: 'text-primary', bg: 'bg-primary/10' },
...
onMounted(async () => {
  try {
    const [venuesRes, categoriesRes, usersRes] = await Promise.all([
      venueRoutes.list(1),
      categoryRoutes.list(1),
      userRoutes.list(1),
    ])
    stats.value[0].value = String(venuesRes.data.meta.total)
    ...
    <div class="grid gap-4 sm:grid-cols-3">
      <Card v-for="stat in stats" :key="stat.title" class="transition-shadow hover:shadow-md">
        <CardHeader class="flex flex-row items-center justify-between pb-2">
          <CardTitle class="text-sm font-medium text-muted-foreground">{{ $t('components.sidebar.' +
            stat.title.toLowerCase()) }}</CardTitle>
      ...
      <Card class="hover:shadow-md transition-shadow">
        ...
        <CardContent class="grid grid-cols-2 gap-3">
          <RouterLink v-for="link in [
            { label: $t('views.dashboard.admin.actions.addVenue'), to: { name: 'admin-venues-create' } },
            { label: $t('views.dashboard.admin.actions.addCategory'), to: { name: 'admin-categories-create' } },
            { label: $t('views.dashboard.admin.actions.allVenues'), to: { name: 'admin-venues' } },
